...
var OllamaUnknownToolError = class extends OllamaClientError {
  toolName;
  constructor(message, options) {
...
var OllamaToolExecutionError = class extends OllamaClientError {
  toolName;
  constructor(message, options) {
...
var OllamaToolValidationError = class extends OllamaClientError {
  toolName;
...
var OllamaAgentMaxIterationsError = class extends OllamaClientError {
  maxIterations;
  constructor(message, options) {
...
var OllamaMcpError = class extends OllamaClientError {
  mcpMethod;
  toolName;
  constructor(message, options) {
...
function toolToOllamaFormat(tool) {
  const parameters = tool.parameters instanceof z.ZodType ? zodToOllamaFormat(tool.parameters) : tool.parameters;
  return {
    type: "function",
    function: {
      name: tool.name,
...
function errorMessage(toolName, error) {
  return {
    role: "tool",
    tool_name: toolName,
...
var ToolRegistry = class _ToolRegistry {
  ...
  filter(names) {
    ...
    return new _ToolRegistry(subset, { onError: this.onError });
  }
  async executeOne(toolCall, ctx) {
    ...
    try {
      ...
      const error = cause instanceof OllamaClientError ? cause : new OllamaToolExecutionError(`Tool "${name}" handler threw while executing.`, {
        toolName: name,
        cause
      });
      return { name, ok: false, error, message: errorMessage(name, error) };
...
var Agent = class {
  ...
  runStream(input) {
    return this.loop(input, async (turn, request) => {
      ...
      if (final.message.thinking !== void 0) message.thinking = final.message.thinking;
      if (final.message.tool_calls !== void 0) message.tool_calls = final.message.tool_calls;
      return message;
    });
  }
  baseRequest(input, messages) {
    return {
      model: input.model,
      messages,
  ...
  async loop(input, getAssistantMessage) {
...
function renderContent(result) {
  return result.content.map(
    (block) => block.type === "text" && typeof block.text === "string" ? block.text : `[unsupported content type: ${block.type}]`
  ).join("\n");
}
async function loadMcpTools(mcpClient, options = {}) {
  let listed;
  try {
  ...
  return listed.tools.map((descriptor) => {
...
async function registerMcpTools(registry, mcpClient, options = {}) {
...
export { Agent, ConsoleLogger, DEFAULT_BASE_URL, DEFAULT_FAILOVER_CODES, DEFAULT_RETRY_CONFIG, DEFAULT_TIMEOUT_MS, EndpointRegistry, MiddlewarePipeline, OllamaAbortError, OllamaAdapter, OllamaAgentMaxIterationsError, OllamaAuthError, OllamaClient, OllamaClientError, OllamaGenericClientError, OllamaMcpError, OllamaNetworkError, OllamaNotFoundError, OllamaRateLimitError, OllamaServerError, OllamaSkillInvalidError, OllamaSkillNotFoundError, OllamaStream, OllamaTimeoutError, OllamaToolExecutionError, OllamaToolValidationError, OllamaUnknownToolError, OllamaUnsupportedFeatureError, OllamaValidationError, RawHttpClient, ToolRegistry, applySkill, checkEndpointHealth, computeBackoffDelayMs, createEnhancedFetch, defineTool, detectModelCapabilities, extractUsage, inferRuntimeMode, listAvailableModels, loadMcpTools, mapError, noopLogger, normalizeChatStream, normalizeGenerateStream, normalizeProgressStream, normalizeRetryConfig, parseFrontmatter, parseStructuredOutput, registerMcpTools, toolToOllamaFormat, zodToOllamaFormat };
